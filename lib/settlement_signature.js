...
function b64UrlSafeEncode(buffer) {
    return buffer.toString('base64').replace(/\+/g, '-').replace(/_/g, '_').replace(/=+$/, '');
}
...
function signatureSettlement({
    ...
    const packageCodeStr = packageCode;
    const source = [accessToken, token, signatureTime, paymentFor, paymentMethodUpper, packageCodeStr].join(";") + ";";
...
module.exports = function settlementSignatureHandler(req, res) {
    if (req.method !== 'POST') {
