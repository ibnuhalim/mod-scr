// controllers/dorv5.js (dorv4 versi ewallet)

const MyXL = require("../services/myxl");

const DOR_KEY = "c351aa50-8494-4450-8473-c25e8445fdc2";
const AKRAB_PLP = "6e469cb2-443d-402f-ba77-681b032ead6a";

async function getBypassPackage(myxl) {
    const packageList = await myxl.getPackageList(AKRAB_PLP);
    const varOrder = 0;
...
module.exports = async function dorv5(req, res) {
    ...
    try {
        ...
        if (!refreshToken || !packageCode || !dorKey || !paymentMethod) {
            return res.status(400).json({
                error: 'Fields "refreshToken", "dorKey", "paymentMethod", and "packageCode" are required.\n\npaymentMethod only support ewallet, QRIS!'
            });
        }
        ...
        const needBypassed = needBypass === true;
        if (needBypassed) {
